...
/**
    ...
    A mixed factory that represents both a <code>ContextFactoryIfc</code> as well as
    a normal <code>FactoryIfc</code> (on that simply creates objects with no context
    invloved.

    This factory defines the "Gang of Four" Factory pattern:  <i>"Define an
    ...
    objects (via the <code>destroy()</code> method).
...
public final class PersonFactory implements MixedFactoryIfc &lt;Person, String&gt;
{
    ...
    <ul>
        ...
        <li>
            Create a factory class implementing <code>MixedFactoryIfc</code> and
            provde the generic type for <code>MixedFactoryIfc</code> (for instance
            <code>implements FactoryIfc &lt;Person, String&gt;</code> above).
        </li>
    </ul>
...
public interface MixedFactoryIfc <V, C>
    extends Serializable, FactoryIfc <V>, ContextFactoryIfc <V, C>
{
    // ----------------------------------------------------------------------
    //
